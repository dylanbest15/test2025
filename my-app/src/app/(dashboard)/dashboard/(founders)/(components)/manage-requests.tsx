...
interface ManageRequestsProps {
  ...
  onApproveInvestment?: (investmentId: string) => void
...
export default function ManageRequests({
  ...
  const handleApproveClick = (investment: JoinedInvestment, event: React.MouseEvent) => {
  ...
  const handleConfirmApproval = () => {
    if (selectedInvestment && onApproveInvestment) {
      onApproveInvestment(selectedInvestment.id)
    }
                  ...
                  {pendingInvestments.map((investment) => (
                    <div key={investment.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex flex-col">
                        ...
                        <Badge className="text-xs bg-yellow-100 text-yellow-800 w-fit mt-1">Pending</Badge>
                      </div>
                      <div className="flex items-center">
                        <div className="flex flex-col space-y-1">
                          <Button
                            ...
                            onClick={(e) => handleApproveClick(investment, e)}
                          >
      ...
      <Dialog open={showApproveModal} onOpenChange={setShowApproveModal}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Are you sure you want to approve this investment request?</DialogDescription>
          </DialogHeader>
