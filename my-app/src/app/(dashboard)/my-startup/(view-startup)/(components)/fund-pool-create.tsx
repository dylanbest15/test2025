...
interface FundPoolCreateProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onSubmit: (amount: number) => void
}

export function FundPoolCreate({ open, onOpenChange, onSubmit }: FundPoolCreateProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]" aria-describedby={undefined}>
        <DialogHeader>
          ...
          <DialogDescription>Set a funding goal for your new fund pool.</DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          <div className="space-y-2">
            <Label htmlFor="amount">Funding Goal Amount</Label>
